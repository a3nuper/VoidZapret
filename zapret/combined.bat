...
chcp 65001 > /dev/null
:: 65001 - UTF-8
:: VoidZapret: combined (base) - web + Discord + Dead by Daylight + YouTube in one winws instance
...
--filter-tcp=443 --hostlist-domains=%DISCORD% --dpi-desync=multisplit --dpi-desync-split-seqovl=568 --dpi-desync-split-pos=1 --dpi-desync-split-seqovl-pattern="%BIN%tls_clienthello_4pda_to.bin" --new ^
...
--filter-tcp=443 --hostlist="%LISTS%list-google.txt" --ip-id=zero --dpi-desync=multisplit --dpi-desync-split-seqovl=568 --dpi-desync-split-pos=1 --dpi-desync-split-seqovl-pattern="%BIN%tls_clienthello_4pda_to.bin" --new ^
